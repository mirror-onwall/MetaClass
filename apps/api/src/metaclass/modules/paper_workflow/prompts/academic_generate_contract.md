# Stage 4：academic-pptx-generate

使用 `academic-pptx-generate` Skill 严格渲染已经冻结的学术汇报合同。不得重新分析论文、改变页面顺序、修改 claim、增加定量结论或替换 asset id。
...
页面标题应与 presentation_outline.json 高度一致。所有 shape 必须位于页面边界内，正文应使用可读字号且不得明显溢出。
