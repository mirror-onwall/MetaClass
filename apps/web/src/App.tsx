...
function App() {
  ...
  return (
    <div className="classroom-app" data-theme={theme}>
      ...
      <main id="top" className="classroom-layout">
        <aside className="left-console">
          ...
          <section className="quick-actions">
            ...
            {presentationMode !== "paper_deck" && <button disabled={!presentationPlan || presentationPlan.mode === "source_deck" || !!busy} onClick={generatePresentationArtifact}><span>03</span><b>{presentationPlan?.mode === "source_deck" ? "使用原 PPT" : presentationArtifact ? "按当前 Plan 重新生成一份 PPT" : "生成 PPT（创建课堂前必需）"}</b><i>↗</i></button>}
      ...
      {(materialProcessingJob?.status === "paused" || ["paused", "failed"].includes(contentJob?.status ?? "") || presentationPlanJob?.status === "paused" || pptJob?.status === "paused" || (pptJob?.status === "failed" && !!pptJob.artifact_id)) && !busy && (
        <div className="paused-job-toast" role="status">
          <span>Ⅱ</span>
          <div><b>{contentJob?.status === "failed" || pptJob?.status === "failed" ? "任务失败，进度已保留" : "进度已保存"}</b><p>{materialProcessingJob?.status === "paused" ? `${materialProcessingJob.message} · ${materialProcessingJob.progress}%` : ["paused", "failed"].includes(contentJob?.status ?? "") ? contentProgressLabel(contentJob!) : presentationPlanJob?.status === "paused" ? presentationProgressLabel(presentationPlanJob) : pptJob?.status === "failed" ? `PPT 生成中断，已完成页面仍可下载 · ${Math.round(pptJob.progress * 100)}%` : "PPT 生成已暂停"}</p></div>
          <button className="resume" type="button" onClick={resumePausedContentWork}>{contentJob?.status === "failed" || pptJob?.status === "failed" ? "从失败处重试" : "从中断处继续"}</button>
